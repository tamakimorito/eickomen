...
export const generateElectricityCommentLogic = (formData: FormData): string => {
    ...
    const shouldStripHyphens = noHyphenProviders.includes(elecProvider);
    const formattedPhone = shouldStripHyphens ? (phone || '').replace(/\D/g, '') : formatPhoneNumberWithHyphens(phone);
    const formattedGasPreContact = shouldStripHyphens ? (gasPreContact || '').replace(/\D/g, '') : formatPhoneNumberWithHyphens(gasPreContact);
...
備考：${elecRemarks || ''}`;
        ...
        case 'ニチガス電気セット':
            ...
            break;
        case 'ユーパワー UPOWER':
            const isDetachedAtInstall = (buildingInfo || '').endsWith('戸建て');
            const upowerHeader = (isNewConstruction === 'はい' || isDetachedAtInstall)
              ? `【U-POWER】【新築再点】`
              : `【U-POWER】`;
