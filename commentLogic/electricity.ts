...
export const generateElectricityCommentLogic = (formData: FormData): string => {
    ...
    if (isSet) {
        ...
        let timePart = gasOpeningTimeSlot || '';

        if (moveInDate && gasOpeningDate && moveInDate === gasOpeningDate) {
            dateLine = `利用開始日(電気/ガス)：${moveInDate} ${timePart}`;
        } else {
    ...
    switch (elecProvider) {
            ...
            switch (elecRecordIdPrefix) {
                ...
                case 'それ以外':
                    if (isVacancy === 'あり') {
                        ...
                        comment = `【ストエネ】${tag}\n契確時間：${elecConfirmationTime || ''}\nレコードID：${recordId || ''}\n主商材受注状況：${primaryProductStatus || ''}\n名乗り：${greeting || ''}\n担当者：${apName || ''}\nプラン：${isAllElectric === 'あり' ? 'すまいのでんきオール電化プラン' : (isGasSet === 'セット' ? 'すまいのでんきセット' : 'すまいの電気のみ')}\n契約者名義（漢字）：${contractorName || ''}\n契約者名義（フリガナ）：${contractorNameKana || ''}\n生年月日(西暦)：${dob || ''}\n電話番号：${phone || ''}\n郵便番号：${postalCode || ''}\n引越し先住所：${address || ''}\n物件名：${buildingInfo || ''}\n${dateLine}\n支払い方法：${paymentMethod || ''}\n備考：${remarks || ''}`;
                    }
                    break;
            }
            break;
        ...
        case '東京ガス電気セット':
            ...
            break;
        case '東邦ガスセット':
            comment = `【東邦ガス電気セット】${tag}\nレコードID：${recordId || ''}\n名乗り：${greeting || ''}\n担当者：${apName || ''}\nプラン：東邦ガスセット\n契約者名義（漢字）：${contractorName || ''}\n契約者名義（フリガナ）：${contractorNameKana || ''}\n生年月日(西暦)：${dob || ''}\n電話番号：${phone || ''}\n郵便番号：${postalCode || ''}\n引越し先住所：${address || ''}\n物件名：${buildingInfo || ''}\n${dateLine}\n支払い方法：${paymentMethod || ''}\n現住所：${currentAddress || '！！必須！！'}\n備考：後確希望時間（${postConfirmationTime || '10-12・12-15・15-18・18-21'}）`;
            break;
        case '大阪ガス電気セット':
