...
export const FormInput = ({ label, name, value, onChange, className = '', isInvalid, ...props }) => {
    ...
    const inputClasses = `block w-full px-3 py-2 bg-white border rounded-lg shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm ${isInvalid ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:ring-emerald-500 focus:border-emerald-500'}`;
    return (
...
export const FormSelect = ({ label, name, value, onChange, options, className = '', isInvalid, ...props }) => {
    const labelClasses = `block text-sm font-bold mb-1 ${isInvalid ? 'text-red-600' : 'text-gray-700'}`;
    const selectClasses = `block w-full px-3 py-2 bg-white border rounded-lg shadow-sm focus:outline-none sm:text-sm ${isInvalid ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:ring-emerald-500 focus:border-emerald-500'}`;
                    ...
                    return (
                        <div key={radioOption.value} className="flex items-center">
                            <input
                                ...
                                className="h-4 w-4 text-emerald-700 border-gray-300 focus:ring-emerald-500"
                            />
...
export const FormTextArea = ({ label, name, value, onChange, className = '', isInvalid, ...props }) => {
    const labelClasses = `block text-sm font-bold mb-1 ${isInvalid ? 'text-red-600' : 'text-gray-700'}`;
    const textareaClasses = `block w-full px-3 py-2 bg-white border rounded-lg shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm ${isInvalid ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:ring-emerald-500 focus:border-emerald-500'}`;
    ...
    return (
        <div className={`relative flex items-start ${className}`}>
            ...
            <input
                ...
                checked={checked}
                onChange={onChange}
                className="h-4 w-4 text-emerald-700 border-gray-300 rounded focus:ring-emerald-500"
                {...props}
            />
            </div>
        ...
        calendarDays.push(
            <button
                ...
                className={`flex items-center justify-center w-9 h-9 text-sm rounded-full transition-colors ${
                    ...
                    isToday ? 'bg-emerald-100 text-emerald-800' : 'text-gray-700 hover:bg-gray-100'
                }`}
            >
                {day}
            </button>
