...
export const FormInput = ({ label, name, value, onChange, className = '', isInvalid, ...props }) => {
    const labelClasses = `block text-sm font-bold mb-1 ${isInvalid ? 'text-red-700' : 'text-red-800'}`;
    const inputClasses = `block w-full px-3 py-2 bg-white border rounded-lg shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm ${isInvalid ? 'border-red-500 focus:ring-red-600 focus:border-red-600' : 'border-green-200 focus:ring-green-600 focus:border-green-600'}`;
    return (
...
export const FormSelect = ({ label, name, value, onChange, options, className = '', isInvalid, ...props }) => {
    const labelClasses = `block text-sm font-bold mb-1 ${isInvalid ? 'text-red-700' : 'text-red-800'}`;
    const selectClasses = `block w-full px-3 py-2 bg-white border rounded-lg shadow-sm focus:outline-none sm:text-sm ${isInvalid ? 'border-red-500 focus:ring-red-600 focus:border-red-600' : 'border-green-200 focus:ring-green-600 focus:border-green-600'}`;
...
export const FormRadioGroup = ({ label, name, value, onChange, options, className = '', isInvalid, ...props }) => {
    const legendClasses = `block text-sm font-bold mb-2 ${isInvalid ? 'text-red-700' : 'text-red-800'}`;
                    ...
                    return (
                        <div key={radioOption.value} className="flex items-center">
                            <input
                                ...
                                className="h-4 w-4 text-green-700 border-green-200 focus:ring-green-600"
                            />
                            <label htmlFor={`${name}-${radioOption.value}`} className="ml-2 block text-sm text-red-900">
                                {radioOption.label}
                            </label>
                        </div>
                    );
...
export const FormTextArea = ({ label, name, value, onChange, className = '', isInvalid, ...props }) => {
    const labelClasses = `block text-sm font-bold mb-1 ${isInvalid ? 'text-red-700' : 'text-red-800'}`;
    const textareaClasses = `block w-full px-3 py-2 bg-white border rounded-lg shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm ${isInvalid ? 'border-red-500 focus:ring-red-600 focus:border-red-600' : 'border-green-200 focus:ring-green-600 focus:border-green-600'}`;
...
export const FormCheckbox = ({ label, name, checked, onChange, description, className = '', isInvalid, ...props }) => {
    const labelClasses = `font-bold ${isInvalid ? 'text-red-700' : 'text-red-800'}`;
    return (
        <div className={`relative flex items-start ${className}`}>
            ...
            <input
                ...
                checked={checked}
                onChange={onChange}
                className="h-4 w-4 text-green-700 border-green-200 rounded focus:ring-green-600"
                {...props}
            />
            </div>
