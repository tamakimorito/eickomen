...
const ManualModal = ({ isOpen, onClose }) => {
  ...
  const SectionTitle = ({ children, icon: Icon }: PropsWithChildren<{ icon?: React.ElementType }>) => (
    <h3 className="text-lg font-bold text-red-700 mt-6 mb-2 border-b-2 border-green-200 pb-1 flex items-center gap-2">
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-3">
            <BookOpenIcon className="h-8 w-8 text-red-700" />
        ...
        <div className="mt-6 text-right">
            <button
              onClick={onClose}
              className="px-6 py-2 bg-red-700 text-white rounded-lg hover:bg-red-800 transition-colors font-semibold"
            >
              閉じる
            </button>
        </div>
