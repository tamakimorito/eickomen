...
const Tab: React.FC<TabProps> = ({ id, label, icon: Icon, activeTab, onTabChange }) => (
    <button
        ...
        className={`flex items-center gap-2 px-3 sm:px-4 py-3 text-sm sm:text-base font-bold transition-colors duration-200 ease-in-out focus:outline-none -mb-px ${
            activeTab === id
            ? 'text-red-700 border-b-4 border-green-600 bg-gradient-to-b from-green-50 to-white rounded-t-xl shadow-inner'
            : 'text-green-800 hover:text-red-700 border-b-4 border-transparent'
        }`}
    >
...
const App = () => {
    ...
    return (
        <div className="bg-gradient-to-b from-green-50 via-white to-green-100 min-h-screen font-sans">
            ...
            <button
                ...
                className="fixed bottom-6 right-6 bg-gradient-to-r from-red-600 to-green-600 text-white p-4 rounded-full shadow-lg hover:from-red-500 hover:to-green-500 transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
