...
## 9. 季節テーマと配色メモ（クリスマス版）
- 2025/12 時点で、UIの基調色を**赤×緑**に切り替えた（ヘッダー/タブ/主要ボタン＝赤と緑のグラデーション、フォームのフォーカス＝緑、ラベル＝赤）。
- セクション背景・枠線は緑系、見出しは赤系で統一。トーストやモーダルの主要ボタンも赤/緑に合わせている。
- 元の配色（復旧用メモ）：
  - ヘッダー/タブ/ボタンの基調はブルー系（例：`bg-blue-700` ヘッダー、タブの選択枠=青）。
  - フォームのフォーカスリングは `focus:ring-blue-500`、セクション背景は `bg-blue-50/50`＋`border-blue-200`、トーストinfoは `bg-blue-500`。
  - クリスマス演出を外す場合は上記ブルー系のクラスへ差し戻す。
