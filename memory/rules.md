...
- **タブ遷移時の住所統合**:
  - 電気/ガス → インターネット：`mailingBuildingInfo`（現住所の物件名＋部屋番号）を、書面発送先未選択時も含め `currentAddress` に重複なく結合。
...
- サカイ販路選択時でも全タブ共通で「レコードID」を必須入力とし、入力欄は非活性化しない。
- ニチガス電気セット/ニチガス単品の営業コメントでは、電話番号・郵便番号をハイフン無しで出力する。
- **ニチガス（電気セット/単品）**：
