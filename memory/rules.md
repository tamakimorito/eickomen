...
- **タブ遷移時の住所統合**:
  - 電気/ガス → インターネット：`mailingOption` の値に関わらず、`mailingBuildingInfo` を `currentAddress` に重複なく結合。
...
- **サカイ販路**：チェックON時もレコードIDは必須（入力不要運用は禁止）。
- **ニチガス（電気セット/単品）**：営業コメントの電話番号・郵便番号はハイフン無しで出力する。
- **ニチガス（電気セット/単品）**：
