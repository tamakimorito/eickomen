...
- **ニチガスの書面送付先**: 「ニチガス単品」「ニチガス電気セット」で書面送付先＝現住所の場合、コメント内の現住所に物件名・部屋番号も連結して表示する。
...
- **ニチガスの書面送付先**: `ニチガス電気セット` および `ニチガス単品` は書面送付先のヒアリング不要（UI非表示・非必須）。
...
- **ニチガス（電気セット/単品）**：`gasArea` は**固定リストからのプルダウン選択**とし、**その他は使用しない**。保存値は表示ラベルと同一の文字列。
- **ニチガス電気セット**：書面送付先のヒアリングは不要。`mailingOption` は**非表示/非必須**。コピー時も未入力で**エラーにしない**。
- `mailingOption==='現住所'` による現住所必須化は **リミックス／東邦ガスセット／東京ガス電気セットのみ**対象。
...
### すまいのでんき（ガスセット）の年末年始不可期間
- 住所の都道府県から関東/関西を判定（山梨は関東、郵便番号による推定は行わない）
- 関東：2025/12/28〜2026/01/09（含む）、関西：2025/12/28〜2026/01/08（含む）
